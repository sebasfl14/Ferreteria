...
public class FacturaMySqlRepositorio implements FacturaRepositorio {
	...
	public int inserta(Factura obj) {
		return jdbcTemplate.update("insert into factura values(null,?,?,?,?)", new Object[] { obj.getNumero_factura(), obj.getFecha_factura(), obj.getTotal_venta(), obj.getEstado_factura()}); 
	...
	public List<Factura> lista(String filtro) {
		List<Factura> lista = jdbcTemplate.query("select * from factura where nombre like ? ",new Object[] { filtro+"%" }, new RowMapper<Factura>() {
            @Override
